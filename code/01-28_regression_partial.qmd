...
# Model fitting
mod1_int <- lmer(yield_mgha ~ 1 + (1|rep), data = reg_dfw)
...
ggplot(mod1_int_aug,
  ...
  geom_hline(yintercept = 0,
             color = "red")+
  geom_smooth() #residuals seem to be increasing,not great as, ideal = when blue lines overlaps red line which would mean residuals are independent.For change, instead of only intercept model, maybe we can add slope to reduce this error.
```
...
ggplot(mod1_int_aug, 
       aes(sample = .stdresid))+
  stat_qq()+#if residuals are normally distributed. Ideally, residuals would fall in the red line.
...
Intercept highly significant!Mean yield is 12.1748 Mg.
...
nd <- nd %>% 
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA#donot consider the random effect, because for random effect, we'd need block, but here we are predicting yield based on seedrate instead of looking at the effect of block
                                   ))
...
```
Intercept changed and seed rate has the slope value.
...
In LP regression, it is estimating slope, intercept and breakeven point.
...
Lack of variance, no need to address random effect for b and xs.
...
```
Based on the linear-plateau model, the level of seeding rate to optimize corn grain yield in this study was **73.47** thousand seeds/ha.   However, the range is from **63.86 to 81.10**

Now let's predict what was the yield at that seeding rate.  
```{r yield at optimum SR}
predict(mod4_linp,
        newdata = data.frame(sr_ksha = 73.48),
        level = 0 #donot incorporate random effect
        )
